...
async function fetchReports() {
  ...
  try {
    ...
    if (!res?.success || !res?.data) {
      throw new Error(res?.message || 'Daftar laporan tidak dapat dimuat.');
    }
    allReports.value = res.data.reports || res.data.complaints || [];
  } catch (e) {
    // Jangan tampilkan tiket lokal sebagai tiket operasional; tiket tersebut
    // tidak pernah masuk ke backend dan tidak bisa ditangani petugas.
    allReports.value = [];
  } finally {
